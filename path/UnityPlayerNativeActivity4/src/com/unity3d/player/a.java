...
final class a {
	...
	private void b(final a a) {
		synchronized (this.g) {
			...
			com.unity3d.player.h.Log(3, String.format(
					"camera: size req = %dx%d \tactual = %dx%d", this.i,
					this.j, this.c.width, this.c.height));
			com.unity3d.player.h.Log(3, String.format(
					"camera: kfps req = %d \tactual = %d-%d", this.k * 1000,
					this.e[0], this.e[1]));
			com.unity3d.player.h.Log(3,
					String.format("camera: parameters [%s]", this.b.flatten()));
			this.a.setParameters(this.b);
			final Camera.PreviewCallback previewCallbackWithBuffer = (Camera.PreviewCallback) new Camera.PreviewCallback() {
				long a2 = 0L;

				public final void onPreviewFrame(final byte[] array,
						final Camera camera) {
					...
					this.a2++;
					if ((a2 & 0xFFL) == 0x0L) {
						com.unity3d.player.h.Log(3, "camera: frame " + this.a2);
					}
					
					
					unityPlayer.onCameraFrame(com.unity3d.player.a.this, array);
				}
			};
	...
	private final Camera.Size f() {
		...
		for (final Camera.Size size : supportedPreviewSizes = this.b
				.getSupportedPreviewSizes()) {
			com.unity3d.player.h.Log(3, String.format("camera: sz[i] = %dx%d",
		...
		for (final Camera.Size size3 : supportedPreviewSizes) {
			final double n4;
			if ((n4 = Math.abs(Math.log(n / size3.width))
					+ Math.abs(Math.log(n2 / size3.height))) < n3) {
				n3 = n4;
				size2 = size3;
			}
		}
	...
	interface aaa {
		void onCameraFrame(com.unity3d.player.a p0, byte[] p1);
	}
}
