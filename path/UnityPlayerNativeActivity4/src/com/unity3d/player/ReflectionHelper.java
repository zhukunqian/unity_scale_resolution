...
final class ReflectionHelper {
	...
	private static boolean a(final a a) {
		final a a2 = ReflectionHelper.a[a.hashCode()
				& ReflectionHelper.a.length - 1];
		if (!a.equals(a2)) {
			return false;
		}
		a.a = a2.a;
	...
	protected static Constructor getConstructorID(final Class clazz,
			final String s) {
		if (ReflectionHelper.LOG) {
			h.Log(3, "? getConstructorID(\"" + clazz.getName() + "\", \"" + s
					+ "\")");
		}
		...
		if (constructor == null) {
			throw new NoSuchMethodError("<init>" + s + " in class "
					+ clazz.getName());
		}
		if (ReflectionHelper.LOG) {
			final StringBuilder sb = new StringBuilder();
			Class[] parameterTypes;
			for (int length2 = (parameterTypes = constructor
					.getParameterTypes()).length, j = 0; j < length2; ++j) {
			...
			h.Log(3, "! " + constructor.getName() + "(" + sb.toString() + ");");
		}
		return constructor;
	}

	protected static Method getMethodID(Class superclass, final String s,
			final String s2, final boolean b) {
		if (ReflectionHelper.LOG) {
			h.Log(3, "? getMethodID(\"" + superclass.getName() + "\", \"" + s
					+ "\", \"" + s2 + "\", " + (b ? "static)" : "non-static)"));
		}
		Method method = null;
		final a a;
		if (a(a = new a(superclass, s, s2))) {
			...
			while (superclass != null) {
				...
				for (int length = (declaredMethods = superclass
						.getDeclaredMethods()).length, i = 0; i < length; ++i) {
					final Method method2 = declaredMethods[i];
					final float a3;
					if (b == Modifier.isStatic(method2.getModifiers())
							&& method2.getName().compareTo(s) == 0
							&& (a3 = a(method2.getReturnType(),
									method2.getParameterTypes(), a2)) > n) {
						method = method2;
						if ((n = a3) == 1.0f) {
							break;
						}
				...
				if (n == 1.0f || superclass.isPrimitive()
						|| superclass.isInterface()
						|| superclass.equals(Object.class)
						|| superclass.equals(Void.TYPE)) {
					break;
				}
		...
		if (method == null) {
			throw new NoSuchMethodError(String.format(
					"no %s method with name='%s' signature='%s' in class L%s;",
					b ? "non-static" : "static", s, s2, superclass.getName()));
		}
		if (ReflectionHelper.LOG) {
			final StringBuilder sb = new StringBuilder();
			Class<?>[] parameterTypes;
			for (int length2 = (parameterTypes = method.getParameterTypes()).length, j = 0; j < length2; ++j) {
	...
	protected static Field getFieldID(Class superclass, final String s,
			final String s2, final boolean b) {
		if (ReflectionHelper.LOG) {
			h.Log(3, "? getFieldID(\"" + superclass.getName() + "\", \"" + s
					+ "\", \"" + s2 + "\", " + (b ? "static)" : "non-static)"));
		}
		Field field = null;
		final a a;
		if (a(a = new a(superclass, s, s2))) {
			...
			while (superclass != null) {
				...
				for (int length = (declaredFields = superclass
						.getDeclaredFields()).length, i = 0; i < length; ++i) {
					final Field field2 = declaredFields[i];
					final float a3;
					if (b == Modifier.isStatic(field2.getModifiers())
							&& field2.getName().compareTo(s) == 0
							&& (a3 = a(field2.getType(), null, a2)) > n) {
						field = field2;
						if ((n = a3) == 1.0f) {
							break;
						}
		...
		if (field == null) {
			throw new NoSuchFieldError(String.format(
					"no %s field with name='%s' signature='%s' in class L%s;",
					b ? "non-static" : "static", s, s2, superclass.getName()));
		}
		if (ReflectionHelper.LOG) {
			h.Log(3,
	...
	private static float a(final Class clazz, Class[] array,
			final Class[] array2) {
		if (array2.length == 0) {
			return 0.1f;
		}
		...
		while (array[0] < s.length()) {
			...
			if ((char1 = s.charAt(array[0]++)) != '(' && char1 != ')') {
				if (char1 == 'L') {
					...
					h.Log(5, "! parseType; " + char1 + " is not known!");
					break;
				}
			}
		}
	...
	private static native Object nativeProxyInvoke(final int p0,
			final String p1, final Object[] p2);
	...
	protected static Object newProxyInstance(final int n, final Class[] array) {
		if (ReflectionHelper.LOG) {
			h.Log(3,
					String.format("ReflectionHelper.Proxy(%d,%s)", n,
							Arrays.asList((Class<?>[]) array)));
		}
		return Proxy.newProxyInstance(ReflectionHelper.class.getClassLoader(),
				array, new InvocationHandler() {
					@Override
					public final Object invoke(final Object o,
							final Method method, final Object[] array) {
						if (ReflectionHelper.LOG) {
							h.Log(3,
									String.format(
											"ReflectionHelper.Proxy.invoke(%d, %s, %s, %s)",
											n, Arrays.asList((Class[]) array),
											method.getName(),
											Arrays.asList(array)));
						}
					...
					protected final void finalize() {
						try {
							if (ReflectionHelper.LOG) {
								h.Log(3,
										String.format(
												"ReflectionHelper.Proxy.finilize(%d, %s)",
												n,
												Arrays.asList((Class[]) array)));
							}
	...
	private static final class a {
		...
		a(final Class b, final String c, final String d) {
			...
			this.c = c;
			this.d = d;
			this.e = 31 * (31 * (527 + this.b.hashCode()) + this.c.hashCode())
					+ this.d.hashCode();
		}
		...
		public final boolean equals(final Object o) {
			...
			if (o instanceof a) {
				final a a = (a) o;
				return this.e == a.e && this.d.equals(a.d)
						&& this.c.equals(a.c) && this.b.equals(a.b);
			}
			return false;
		}
	}
